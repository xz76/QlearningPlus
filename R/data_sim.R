...
simulate_dtr_data <- function(n = 300) {
  ...
  # Stage 1 treatment: 3 arms
  A1 <- sample(1:3, n, replace = TRUE)

  # Stage 2 treatment: 3 arms
  A2 <- sample(1:3, n, replace = TRUE)

  # Covariate-dependent treatment effects
  A1_effect <- ifelse(A1 == 1, 1 + 0.5 * X1,
                      ifelse(A1 == 2, 2 - 0.3 * X2,
                             0.5 + 0.2 * X1 * X2))

  A2_effect <- ifelse(A2 == 1, 1.5 + 0.4 * X2,
                      ifelse(A2 == 2, 2.0 - 0.1 * X1^2,
                             0.8 + 0.3 * abs(X1 - X2)))

  # Outcome
  epsilon <- rnorm(n)
  Y1 <- 2 + 1.2 * X1 + 1.5 * X2 + A1_effect + epsilon
  Y2 <- 4 + 0.5 * X2 + A2_effect + rnorm(n)
